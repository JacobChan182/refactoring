...
public class StatementPrinter {
    ...
    /**
     * StatementPrinter's Constructor.
    ...
    public String statement() {
        ...
        for (Performance p : invoice.getPerformances()) {
            ...
            switch (play.getType()) {
                case "tragedy":
                    ...
                    if (p.getAudience() > Constants.TRAGEDY_AUDIENCE_THRESHOLD) {
                        thisAmount += Constants.HISTORY_OVER_BASE_CAPACITY_PER_PERSON * (p.getAudience() - Constants.BASE_VOLUME_CREDIT_THRESHOLD);
                    }
                    break;
                case "comedy":
            ...
            result.append(String.format("  %s: %s (%s seats)%n", play.getName(), frmt.format(thisAmount / Constants.PERCENT_FACTOR), p.getAudience()));
